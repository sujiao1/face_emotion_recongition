...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, params, num_classes=8):
        self.inplanes = 64
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=s2, padding=3, dilation=d1,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=s2, padding=d2, dilation=d2)
        self.layer1 = self._make_layer(block, 64, layers[0], stride=s2, dilation=d2)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=s3, dilation=d3)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=s4, dilation=d4)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=s5, dilation=d5)
...
def test_se_resnet_50(params,pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SENet(Bottleneck, [3, 4, 6, 3], params,**kwargs)
    return model
...
def main():
    ...
    params.output_stride = 8
    model = test_se_resnet_50(params)
    print(model)
